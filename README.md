...
`OddsStreamService`（actor）以 `Task.sleep` loop 模擬持續推播，透過 `nonisolated let` 的 `PassthroughSubject` 對外暴露 `updates` Publisher。Repository 持有串流實體並暴露 `oddsPublisher`（不 sink），ViewModel 訂閱後以 `collect(.byTime(..., 100ms))` 批次合併高頻事件，再透過 `CurrentValueSubject` 推送給 ViewController。
...
**串流層 — `OddsStreamService` actor**：所有可變狀態（`lastKnownOdds`、`reconnectDelay`、`reconnectAttempts`、Task handles）均受 actor serial executor 保護。計時循環與斷線模擬統一使用 `Task` + `Task.sleep`，消除原先的 `DispatchWorkItem` / `DispatchQueue.main.asyncAfter` 混用。`nonisolated let subject` 讓外部可同步訂閱 Publisher，`send()` 僅從 actor 方法呼叫，序列化由 actor 隱式保證。

**資料層 — `OddsRepository` actor**：`OddsRepository` 宣告為 `actor`，擁有自己的 background serial executor。所有資料處理（merge、sort、applyUpdates、cachedList 讀寫）都在此 executor 序列執行，透過 actor isolation 靜態保證不會有兩段程式碼同時讀寫同一份資料——無需手動加鎖，無 Race Condition。

**UI 層 — `@MainActor OddsListViewModel`**：ViewModel 宣告 `@MainActor`，所有屬性（`cancellables`、Subjects）與方法均保證在主線程執行。從 actor（Repository）`await` 回來後自動回到 MainActor，無需 `await MainActor.run { }`。Combine sink 閉包不是 `@MainActor` context，寫 Subject 前以 `Task { @MainActor in }` 顯式回到主線程。

**單一資料源**：比賽列表由 `CurrentValueSubject<[MatchCellModel], Never>` 持有，外部只能訂閱，無法直接修改。

**資料流**：WebSocket batch → `collect(100ms)` 批次合併（main queue）→ `Task { @MainActor in await repository.applyUpdates(...) }`（actor background）→ Subject send（MainActor）
...
`OddsStreamService`（actor）模擬每 15 秒斷線一次，內部以 `Task.sleep` 計時（取代 DispatchWorkItem），斷線時透過 `disconnectedSubject` 發出事件。Repository 暴露 `disconnectedPublisher`，ViewModel 訂閱後呼叫 `repository.reconnectStream()`，重連延遲從 1 秒開始每次加倍（1s → 2s → 4s → 8s → 16s → 上限 30s），設定最大重連次數（10 次）防止無限重試。連線成功後延遲與次數自動重置。
...
├── ViewModels/
│   └── OddsListViewModel.swift      ← @MainActor；訂閱 Repository Publisher、collect 100ms 批次、changesPublisher
├── ViewControllers/
...
├── Repositories/
│   └── OddsRepository.swift         ← actor；fetch + merge + sort + cache + applyUpdates + 持有 stream（不訂閱）
└── Services/
    ...
    └── OddsStreamService.swift       ← actor；Task.sleep loop 模擬 WebSocket + Exponential Backoff 重連
...
`StubStreamService` 為 `final class`，實作 `OddsStreamProtocol` 的 `async` 方法時提供同步實作（Swift 允許非 async 函式滿足 async 協定需求），測試端可直接同步讀取 `isStarted` / `isPaused` / `reconnectCount`。
